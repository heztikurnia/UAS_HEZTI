<x-guest-layout>
    <form method="post" action="{{ route('songs.update', $song->id) }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')
        ...
        <div>
            ...
            <x-text-input id="duration" name="duration" type="text" min="1" class="mt-1 block w-full" :value="old('duration', $song->duration)" required />
            <x-input-error class="mt-2" :messages="$errors->get('duration')" />
        </div>

        <div>
            <x-input-label for="genre" :value="__('Genre')" />
            <x-text-input id="genre" name="genre" type="text" class="mt-1 block w-full" :value="old('genre', $song->genre)" />
            <x-input-error class="mt-2" :messages="$errors->get('genre')" />
        </div>

        <div>
            <x-input-label for="description" :value="__('Description')" />
            <x-text-input id="description" name="description" type="text" class="mt-1 block w-full" :value="old('description', $song->description)" />
        ...
        <div>
            ...
            <select id="status" name="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                <option value="0" {{ old('status', $song->status) == 0 ? 'selected' : '' }}>{{ __('Valid') }}</option>
                <option value="1" {{ old('status', $song->status) == 1 ? 'selected' : '' }}>{{ __('Invalid') }}</option>
                <option value="2" {{ old('status', $song->status) == 2 ? 'selected' : '' }}>{{ __('not valid') }}</option>
            </select>
